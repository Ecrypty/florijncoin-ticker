...
from config.rkeys import r_SS_FLRN_BTC_5MIN_HISTORY

def make_request():
    startepoch = '1391806500'
    URL        = "https://poloniex.com/public?command=returnChartData&currencyPair=BTC_FLRN&start=" + startepoch + "&end=9999999999&period=300"
...
def getload():
    ...
    for x in data:
        ...
        try:
            pipe = r.pipeline()
            pipe.zadd(r_SS_FLRN_BTC_5MIN_HISTORY, date, str(date) + ':' + str(Avg))
...
#-------------------------------------------------------------------
# FILE TO SAVE
filetow = 'florijncoin_poloniex_history.log'

print('Keys is : %s' % r_SS_FLRN_BTC_5MIN_HISTORY)

# redis
POOL = redis.ConnectionPool(host='localhost', port=6379, db=0)
r = redis.StrictRedis(connection_pool=POOL)

print(r.zcard(r_SS_FLRN_BTC_5MIN_HISTORY))
name = input("Do you want delete key first: [Yes/No] ")
if name == 'Yes':
    r.delete(r_SS_FLRN_BTC_5MIN_HISTORY)
    print(r.zcard(r_SS_FLRN_BTC_5MIN_HISTORY))
else:
    sys.exit()
